...
class EnhancedConstraintResolver:
    ...
    def _find_better_day_for_subject(self, entries: List[TimetableEntry], class_group: str, subject: Subject, exclude_day: str) -> str:
        """Find a better day for a subject that does not have this subject yet."""
        config = self._get_schedule_config()
        if not config:
            return None
    ...
    def _can_move_entry_to_period(self, entry: TimetableEntry, entries: List[TimetableEntry], day: str, period: int) -> bool:
        """Check if an entry can be moved to a specific day and period."""
        # Check if the slot is available for this class group
        if not self._is_slot_available(entries, entry.class_group, day, period):
            return False
        
        # Check if teacher is available
        teacher_conflict = any(
            e.teacher == entry.teacher and e.day == day and e.period == period and e != entry
            for e in entries
        )
        if teacher_conflict:
            return False
        ...
        room_conflict = any(
            e.classroom == entry.classroom and e.day == day and e.period == period and e != entry
            for e in entries
        )
        if room_conflict:
            return False
    ...
    def _is_slot_available(self, entries: List[TimetableEntry], class_group: str, day: str, period: int) -> bool:
        ...
        return not any(
            e.class_group == class_group and e.day == day and e.period == period
            for e in entries
        )
    ...
    def _create_entry(self, day: str, period: int, subject: Subject, teacher: Teacher, 
        ...
        # Calculate actual start time for this period
        total_minutes = (period - 1) * (config.class_duration + 15)
        actual_start_time = (
            timedelta(hours=start_time.hour, minutes=start_time.minute) +
            timedelta(minutes=total_minutes)
        )
    ...
    def _find_appropriate_teacher_for_subject(self, subject: Subject, class_group: str) -> Optional[Teacher]:
        """Find an appropriate teacher for a subject."""
        if not subject:
            return None
        
        # Try to find a teacher assignment for this subject
    ...
    def _find_available_period_on_day(self, entries: List[TimetableEntry], class_group: str, day: str) -> Optional[int]:
        """Find an available period on a specific day."""
        config = self._get_schedule_config()
        if not config:
            return None
